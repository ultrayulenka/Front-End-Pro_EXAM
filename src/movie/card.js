...
export class MovieCard extends BaseUiComponent {
  ...
  onClick(event) {
    ...
    if (
      ...
      event.target.closest("button").classList.contains("btn-delete")
    ) {
      const confirmation = confirm(
        `Вы действительно хотели бы удалить фильм "${this.movie.title}"?`
      );
      if (confirmation) {
    ...
    if (
      ...
      event.target.closest("button").classList.contains("btn-edit")
    ) {
