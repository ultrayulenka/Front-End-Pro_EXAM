...
export class Modal extends BaseUiComponent {
    constructor(data) {
        ...
        if(data) {
            this.id = data;
            this._element.classList.add(this.id);
        }
    }
    ...
    onSave(){
        ...
        const newFields = this._element.querySelectorAll(".new-field");
            newFields.forEach(field => {
                const newPosition ={};
                newPosition[field.querySelector("input[name=position]").value.toLowerCase()] = field.querySelector("input[name=name]").value;
    ...
    fillInFields(){
        ...
        if(movie.additionalPositions.length>0){
            ...
            newFields.forEach((field,index) => {
                field.querySelector("input[name=position]").value = Object.keys(movie.additionalPositions[index])[0];
                field.querySelector("input[name=name]").value = Object.values(movie.additionalPositions[index])[0];
                })
            }
    }
